...
use crate::strategy::{
    HistoricalReplayStore, IndicatorSpec, InMemoryPriceStore, PriceStore, PriceUpdated,
    SharedTimeframeEngine, StrategyContext, StrategyError, SsuConfig, TimeframeEngine,
    TimeframeUpdate,
};
...
pub trait StrategyPositionBook: Send + Sync {
    ...
    fn close_position(&self, signal: &ExitSignal) -> Result<StrategyPosition, StrategyError>;
}
...
impl StrategyPositionBook for SqliteStrategyPositionBook {
    ...
    fn open_position(
        ...
        let position = StrategyPosition {
            position_id: format!("VP-{}", state.next_id),
            ssu_id: signal.ssu_id,
            trigger_instrument: signal.trigger_instrument.clone(),
            trade_instrument: signal.trade_instrument.clone(),
            side: signal.side,
    ...
    fn close_position(&self, signal: &ExitSignal) -> Result<StrategyPosition, StrategyError> {
        let mut state = self.state.lock().expect("position store lock poisoned");
        let Some(position_id) = state
            .positions
            .values()
            .filter(|position| {
                position.ssu_id == signal.ssu_id
                    && position.trade_instrument == signal.trade_instrument
            ...
            return Err(StrategyError::Rule(format!(
                ...
                signal.ssu_id, signal.trade_instrument
            )));
        };
        let position = state
            .positions
        ...
        position.exit_price = Some(signal.price);
        position.exit_at = Some(signal.at);
        position.exit_reason = Some(signal.reason.clone());
        position.pnl = Some(compute_pnl(position.side, position.entry_price, signal.price));
...
impl SignalSink for TelegramSignalSink {
    fn consume(&self, signal: &StrategySignal) -> Result<(), StrategyError> {
        let component = match signal {
            StrategySignal::Entry(_) => "STRATEGY:SIGNAL:ENTRY",
            StrategySignal::Exit(_) => "STRATEGY:SIGNAL:EXIT",
            StrategySignal::Shift(_) => "STRATEGY:SIGNAL:SHIFT",
            StrategySignal::Rollover(_) => "STRATEGY:SIGNAL:ROLLOVER",
        };
...
impl StrategyRuntime {
    ...
    pub fn reload_ssus(&self) -> Result<usize, StrategyError> {
        let _gate = self.lifecycle_gate.write().expect("strategy lifecycle gate poisoned");
        ...
        for config in configs {
            ...
            let mut required = config.required_timeframes.iter().copied().collect::<BTreeSet<_>>();
            ...
            for instrument in &self.warmup_spot_instruments {
                for timeframe in &required {
                    ...
                    let bars = if existing.is_empty() {
                        self.historical
                            .load_bars(instrument, *timeframe, self.warmup_bars.max(1))?
                    } else {
                        existing
                    };
                    if !bars.is_empty() {
    ...
    pub fn on_tick(
        ...
        let _gate = self.lifecycle_gate.read().expect("strategy lifecycle gate poisoned");
...
pub fn start_strategy_runtime(
    ...
    let repository = Arc::new(SqliteSsuRepository::new(strategy_config.sqlite_path.clone())?);
...
fn parse_status(value: &str) -> Result<PositionStatus, StrategyError> {
    match value.trim().to_ascii_uppercase().as_str() {
        ...
        value => Err(StrategyError::Parse(format!("unsupported position status {value}"))),
    }
}
...
fn signal_label(signal: &StrategySignal) -> &'static str {
    match signal {
        StrategySignal::Entry(_) => "ENTRY",
        StrategySignal::Exit(_) => "EXIT",
        StrategySignal::Shift(_) => "SHIFT",
        StrategySignal::Rollover(_) => "ROLLOVER",
    }
}

fn signal_message(signal: &StrategySignal) -> String {
    match signal {
        StrategySignal::Entry(signal) => format!(
            "ENTRY | ssu={} | instrument={} | side={} | price={:.4} | reason={} | at={}",
            signal.ssu_id,
            signal.trade_instrument,
            side_label(signal.side),
            signal.price,
            signal.reason,
            signal.at
        ),
        StrategySignal::Exit(signal) => format!(
            "EXIT | ssu={} | instrument={} | side={} | price={:.4} | reason={} | at={}",
            signal.ssu_id,
            signal.trade_instrument,
            side_label(signal.side),
            signal.price,
        ...
        StrategySignal::Shift(signal) => format!(
            "SHIFT | ssu={} | from={} | to={} | side={} | price={:.4} | reason={} | at={}",
            signal.ssu_id,
            signal.trade_instrument,
            signal.next_instrument,
            side_label(signal.side),
            signal.price,
            signal.reason,
            signal.at
        ),
        StrategySignal::Rollover(signal) => format!(
            "ROLLOVER | ssu={} | from={} | to={} | side={} | price={:.4} | reason={} | at={}",
            signal.ssu_id,
            signal.trade_instrument,
            signal.next_instrument,
            side_label(signal.side),
...
mod tests {
    ...
    fn position_store_enforces_ssu_limits() {
        let store = SqliteStrategyPositionBook::new(temp_sqlite("strategy-position-store"))
            .expect("store");
        let ssu = SsuConfig {
            ssu_id: 11,
        ...
        let signal = EntrySignal {
            ssu_id: 11,
            trigger_instrument: "NIFTY".to_string(),
            trade_instrument: "NIFTY".to_string(),
    ...
    fn runtime_dispatches_spot_trigger_to_all_active_ssus() {
        ...
        let runtime = StrategyRuntime::new(
            ...
            Arc::new(FakeFactory {
                strategy: Arc::new(CountingStrategy { calls: Arc::clone(&calls) }),
            }),
    ...
    fn candle_cycle_strategy_enters_exits_and_reenters_after_cooldown() {
        ...
        let positions = Arc::new(
            SqliteStrategyPositionBook::new(sqlite_path.clone()).expect("positions"),
        );
        let runtime = StrategyRuntime::new(
            Arc::new(FakeRepository {
                ssus: vec![SsuConfig {
